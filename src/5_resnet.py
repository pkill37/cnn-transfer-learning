...
def resnet50(l2):
    ...
    resnet50 = tf.keras.applications.resnet50.ResNet50(weights='imagenet', input_shape=(IMG_WIDTH, IMG_HEIGHT, IMG_CHANNELS), include_top=False)
    ...
    model = tf.keras.models.Model(inputs=resnet50.input, outputs=y)
    model.compile(loss=LOSS, optimizer=OPTIMIZER, metrics=METRICS)
    return model


def train_resnet50(experiments, x, y, epochs, batch_size):
...
def main(experiments, train_set, epochs, batch_size):
    ...
    train_resnet50(experiments, x, y, epochs, batch_size)
