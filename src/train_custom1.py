...
def custom1(l2, unitsA, unitsB, dropoutA, dropoutB):
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 3), strides=(1, 1), padding='same', activation='relu', kernel_regularizer=tf.keras.regularizers.l2(l2)), tf.keras.layers.MaxPooling2D(pool_size=(2, 2)),
        ...
        tf.keras.layers.Dense(units=unitsA, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(l2)),
        tf.keras.layers.Dropout(rate=dropoutA),
        tf.keras.layers.Dense(units=unitsB, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(l2)),
        tf.keras.layers.Dropout(rate=dropoutB),
        tf.keras.layers.Dense(units=1, activation='sigmoid', kernel_regularizer=tf.keras.regularizers.l2(l2)),
    ])
    return model


def main(experiments, train_set, epochs, batch_size, l2, unitsA, unitsB, dropoutA, dropoutB, patience):
    ...
    # Settings
    run = os.path.join(experiments, f'lambda{l2}_unitsA{unitsA}_unitsB{unitsB}_dropoutA{dropoutA}_dropoutB{dropoutB}_patience{patience}')
    ...
    # Train
    model = custom1(l2, unitsA, unitsB, dropoutA, dropoutB)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--unitsA', type=int, required=True)
    parser.add_argument('--unitsB', type=int, required=True)
    parser.add_argument('--dropoutA', type=float, required=True)
    parser.add_argument('--dropoutB', type=float, required=True)
    parser.add_argument('--patience', type=int, required=True)
    args = parser.parse_args()
    main(args.experiments, args.train_set, args.epochs, args.batch_size, args.l2, args.unitsA, args.unitsB, args.dropoutA, args.dropoutB, args.patience)
