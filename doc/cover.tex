...
\Title{\selectlanguage{portuguese}Deep Learning para Imagem Médica}
...
\Title{\selectlanguage{english}Deep Learning for Medical Imaging}
...
\SecondSupervisor{Pétia Georgieva}{auxiliar convidado}{do Departamento de Eletrónica, Telecomunicações e Informática}{da Universidade de Aveiro}
...
\Abstract{Keywords}{medical imaging, melanoma, binary classification, deep learning, convolutional neural network, ISIC, transfer learning, ImageNet, VGG16, InceptionV3.}
         {Abstract}{\selectlanguage{english}Transfer learning is a popular solution to a common problem in deep learning that is the lack of data or the computational resources to train large models from scratch, which skin lesion classification is a prime candidate for because high quality medical imaging data in this domain is scarce. This dissertation studies transfer learning in the domain of skin lesion classification by exploring pre-trained models of architectures like VGG16 and InceptionV3 (originally trained on ImageNet) and repurposing them for skin lesion classification on the ISIC 2018 dataset. Models of VGG16 are tested by exhausting the layers at which weights are extracted from and up to which they are frozen from further training. Results show that extracting all layers from VGG16 and fine-tuning them to the ISIC 2018 dataset is the most performant.}
